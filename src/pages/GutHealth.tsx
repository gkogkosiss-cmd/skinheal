...
import { Heart, Microscope, ShieldCheck, Pill, Utensils } from "lucide-react";
...
const supplements = [
  ...
  { name: "Magnesium (200–400 mg/day)", benefit: "Supports nervous system, sleep quality, and reduces stress hormones" },
      ...
      <div className="space-y-8">
        ...
        <div className="card-elevated">
          ...
          <div className="space-y-6">
            {gutHealingFoods.map((g) => (
              <div key={g.category}>
                <h3 className="font-medium text-sm mb-2">{g.category}</h3>
                <p className="text-xs text-muted-foreground mb-2">{g.note}</p>
                <div className="flex flex-wrap gap-2">
                  {g.items.map((item) => (
                    <span key={item} className="px-3 py-1.5 rounded-full bg-accent text-xs font-medium text-accent-foreground">
                      {item}
                    </span>
                  ))}
                </div>
